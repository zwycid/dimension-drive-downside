...
//		coverFlow.setMaxZoom(-120); // original value
		...
		coverFlow.setOnItemSelectedListener(new OnItemSelectedListener(){
			public void onItemSelected(
				...
				//Worning : 아래에 �같은 내용이 있는데, 항상 같아야 합니다!!(바뀔쌔 쓰이는 메소드들//터치로 맵 바꿀)
				long value = 0;
				value = getEvent(P_TIME,albumNumber);
				textviewScore.setText("Top Score : " + ((value==-1)?" No data " : value+" Points"));
				value = getEvent(SCORE,albumNumber);
		...
		//TODO : 여기에 DB에서 값을 가져와 넣으면 됨!!
		//Worning : 위에 �같은 내용이 있는데, 항상 같아야 합니다!!(처음 보여줄 때 쓰는 메소드들)
		long value = 0;
		value = getEvent(P_TIME,albumNumber);
		textviewScore.setText("Top Score : " + ((value==-1)?" No data " : value+" Points"));
		value = getEvent(SCORE,albumNumber);
...
//            i.setLayoutParams(new CoverFlow.LayoutParams(130, 130)); // original value
	...
	private long getEvent(String column, int mapid){
    	...
    	String sql = "SELECT "+column+" FROM "+TABLE_NAME+" WHERE "+column+"=? ORDER BY "+column+" DESC;";
	...
	//ptime : 플레이 시간
	//score : 점수  
	private void addEvents(int ptime,int score){
		SQLiteDatabase db = events.getWritableDatabase();
    	ContentValues values = new ContentValues();
    	
    	values.put(P_TIME, ptime);
